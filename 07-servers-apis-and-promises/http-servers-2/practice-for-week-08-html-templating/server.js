...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === "GET" && req.url === "/dogs/new") {
      const htmlTemplate = fs.readFileSync("./views/create-dog.html");

      let resBody = htmlTemplate;
      res.status = 200;
      res.setHeader("Content-Type", "text/html");
      res.write(resBody);
      return res.end();
    ...
    if (req.method === "POST" && req.url === "/dogs") {
      let newDog = { dogId: getNewDogId(), ...req.body };
      dogs.push(newDog);

      res.writeHead(302, {
        location: `/dogs/${newDog.dogId}`,
      });
      return res.end();
    }
    ...
    if (req.method === "POST" && req.url.startsWith("/dogs/")) {
      ...
      if (urlParts.length === 3) {
        ...
        const dogPos = dogs.indexOf(dog);
        dogs[dogPos].name = req.body.name;
        dogs[dogPos].age = req.body.age;

        res.writeHead(302, {
          location: `/dogs/${dog.dogId}`,
        });
        return res.end();
      }
    }
